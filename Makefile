...
CFLAGS=-m64 -c -Wall -g3 -gdwarf-2 -D DEBUG -I/usr/include/mysql -I./src $(DEFS)
LDFLAGS=
...
BUILDPATH=./build
INSTALL_HEADER_PATH=/usr/local/include/libpeerconnectorcore
INSTALL_LIB_PATH=/usr/local/lib
DST_HEADERS=$(subst ./src,$(INSTALL_HEADER_PATH),$(HEADERS))
STATIC_LIBRARY=libnrcore.a
...
install:$(DST_HEADERS)
	...
	cp $(BUILDPATH)/$(EXECUTABLE) $(INSTALL_LIB_PATH)/$(EXECUTABLE)
...
clean:
	rm -Rf $(BUILDPATH)/*.o
	rm -f $(BUILDPATH)/$(EXECUTABLE)

remove:
	rm -Rf $(INSTALL_HEADER_PATH)
	rm -f $(INSTALL_LIB_PATH)/$(EXECUTABLE)
