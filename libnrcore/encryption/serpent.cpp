...
void Serpent::init(const void* key, uint16_t keysize_b){
	uint32_t buffer[8];
	uint8_t i,j;
	if(keysize_b<256){
...
void Serpent::encrypt(void *buffer, int len)
{
    if (len%16)
        throw "Invalid Length";

    for (int i=0; i<len; i+=16)
        enc(&reinterpret_cast<char*>(buffer)[i]);
}

void Serpent::decrypt(void *buffer, int len)
{
    if (len%16)
        throw "Invalid Length";

    for (int i=0; i<len; i+=16)
        dec(&reinterpret_cast<char*>(buffer)[i]);
}
